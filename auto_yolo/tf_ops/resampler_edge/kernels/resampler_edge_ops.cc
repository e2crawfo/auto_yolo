...
template <typename T>
struct ResamplerEdge2DFunctor<CPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const CPUDevice& d,
    ...
    const T hi_x = static_cast<T>(data_width-1);
    const T hi_y = static_cast<T>(data_height-1);
    ...
    auto resample_batches = [&](const int start, const int limit) {
      for (int batch_id = start; batch_id < limit; ++batch_id) {
        ...
        for (int sample_id = 0; sample_id < num_sampling_points; ++sample_id) {
          const T x = std::min(std::max(warp[batch_id * warp_batch_stride + sample_id * 2], zero), hi_x);
          const T y = std::min(std::max(warp[batch_id * warp_batch_stride + sample_id * 2 + 1], zero), hi_y);
...
class ResamplerEdgeOp : public ::tensorflow::OpKernel {
 public:
  explicit ResamplerEdgeOp(::tensorflow::OpKernelConstruction* context)
  ...
  void Compute(::tensorflow::OpKernelContext* ctx) override {
    ...
    if (num_sampling_points > 0) {
      functor::ResamplerEdge2DFunctor<Device, T>()(
 ...
 private:
  TF_DISALLOW_COPY_AND_ASSIGN(ResamplerEdgeOp);
};

#define REGISTER(TYPE)                                                \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("ResamplerEdge").Device(DEVICE_CPU).TypeConstraint<TYPE>("T"), \
      ResamplerEdgeOp<CPUDevice, TYPE>);
...
#define REGISTER(TYPE)                                                \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("ResamplerEdge").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      ResamplerEdgeOp<GPUDevice, TYPE>)
TF_CALL_float(REGISTER);
TF_CALL_double(REGISTER);
#undef REGISTER
#endif  // GOOGLE_CUDA
...
template <typename T>
struct ResamplerEdgeGrad2DFunctor<CPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const CPUDevice& d,
                  const T* __restrict__ data, const T* __restrict__ warp,
    ...
    const int resampler_edge_output_size =
        batch_size * num_sampling_points * data_channels;
    const int grad_warp_size = resampler_edge_output_size / data_channels * 2;
...
template <typename Device, typename T>
class ResamplerEdgeGradOp : public ::tensorflow::OpKernel {
 public:
  explicit ResamplerEdgeGradOp(::tensorflow::OpKernelConstruction* context)
      : ::tensorflow::OpKernel(context) {}
  ...
  void Compute(::tensorflow::OpKernelContext* ctx) override {
    ...
    ::tensorflow::TensorShape resampler_edge_output_shape = warp.shape();
    resampler_edge_output_shape.set_dim(resampler_edge_output_shape.dims() - 1,
                                   data_channels);
    OP_REQUIRES(ctx, grad_output_shape == resampler_edge_output_shape,
                ::tensorflow::errors::InvalidArgument(
                    "grad_output shape is not consistent with data and warp "
                    "shapes; it should be ",
                    resampler_edge_output_shape.DebugString(), " but is ",
    ...
    // Execute kernel only for nonempty output; otherwise Eigen crashes on GPU.
    if (num_sampling_points > 0) {
      functor::ResamplerEdgeGrad2DFunctor<Device, T>()(
          ctx, ctx->eigen_device<Device>(), data.flat<T>().data(),
 ...
 private:
  TF_DISALLOW_COPY_AND_ASSIGN(ResamplerEdgeGradOp);
};

#define REGISTER(TYPE)                                                    \
  REGISTER_KERNEL_BUILDER(                                                \
      Name("ResamplerEdgeGrad").Device(DEVICE_CPU).TypeConstraint<TYPE>("T"), \
      ResamplerEdgeGradOp<CPUDevice, TYPE>);
...
#define REGISTER(TYPE)                                                    \
  REGISTER_KERNEL_BUILDER(                                                \
      Name("ResamplerEdgeGrad").Device(DEVICE_GPU).TypeConstraint<TYPE>("T"), \
      ResamplerEdgeGradOp<GPUDevice, TYPE>)
