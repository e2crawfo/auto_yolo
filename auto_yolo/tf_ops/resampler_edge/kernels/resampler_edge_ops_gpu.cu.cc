...
template <typename T>
__global__ void ResamplerEdge2DKernel(const T* __restrict__ data,
  ...
  const float hi_x = static_cast<float>(data_width-1);
  const float hi_y = static_cast<float>(data_height-1);
  const float zerof = static_cast<float>(0);
  ...
  CUDA_1D_KERNEL_LOOP(index, output_data_size) {
    ...
    const T x = static_cast<T>(min(max(static_cast<float>(warp[batch_id * warp_batch_stride + sample_id * 2]), zerof), hi_x));
    const T y = static_cast<T>(min(max(static_cast<float>(warp[batch_id * warp_batch_stride + sample_id * 2 + 1]), zerof), hi_y));
...
template <typename T>
struct ResamplerEdge2DFunctor<GPUDevice, T> {
...
// template struct ResamplerEdge2DFunctor<GPUDevice, Eigen::half>;
template struct ResamplerEdge2DFunctor<GPUDevice, float>;
template struct ResamplerEdge2DFunctor<GPUDevice, double>;
...
template <typename T>
__global__ void ResamplerEdgeGrad2DKernel(
    const T* __restrict__ data, const T* __restrict__ warp,
  ...
  CUDA_1D_KERNEL_LOOP(index, resampler_edge_output_size) {
    const int out_index = index;
    ...
    const T x = static_cast<T>(min(max(static_cast<float>(warp[warp_id_x]), zerof), hi_x));
    const T y = static_cast<T>(min(max(static_cast<float>(warp[warp_id_y]), zerof), hi_y));
...
template <typename T>
struct ResamplerEdgeGrad2DFunctor<GPUDevice, T> {
  void operator()(::tensorflow::OpKernelContext* ctx, const GPUDevice& d,
                  const T* __restrict__ data, const T* __restrict__ warp,
                  const T* __restrict__ grad_output, T* __restrict__ grad_data,
    ...
    const int resampler_edge_output_size =
        batch_size * num_sampling_points * data_channels;
    config = ::tensorflow::GetCudaLaunchConfig(resampler_edge_output_size, d);
    ResamplerEdgeGrad2DKernel<T>
        <<<config.block_count, config.thread_per_block, 0, d.stream()>>>(
...
template struct ResamplerEdgeGrad2DFunctor<GPUDevice, float>;
